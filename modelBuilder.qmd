...
trainData = [(np.array(x), np.array(y)) for x,y in zip(X_train, y_train)]
testData = [(np.array(x), np.array(y)) for x,y in zip(X_test, y_test)]
```

```{python}
from scripts.myNeuralNet import Network
import numpy as np
myNetwork = Network([int(i) for i in list(np.linspace(max_vocab_size, 1, 12))])
myNetwork.SGD(training_data = trainData, epochs = 10, mini_batch_size = 10, learningRate = 2, test_data = testData)
